...
export class WasmEmbeddedPropagator {
   ...
   */
  getParamField(field: string, channel: number): Float32Array;
  /**
   * Create embedded propagator with species configurations.
   */
  static newWithSpecies(config_json: string, seed_json: string, species_json: string): WasmEmbeddedPropagator;
  /**
   * Reset with species configurations.
   */
  resetWithSpecies(seed_json: string, species_json: string): void;
  /**
   * Get current simulation state with parameter fields.
   */
  getStateWithParams(): any;
  /**
...
export interface InitOutput {
  ...
  readonly __wbg_wasmembeddedpropagator_free: (a: number, b: number) => void;
  readonly __wbg_wasmgpupropagator_free: (a: number, b: number) => void;
  readonly __wbg_wasmpropagator_free: (a: number, b: number) => void;
  readonly wasmembeddedpropagator_getHeight: (a: number) => number;
  readonly wasmembeddedpropagator_getParamField: (a: number, b: number, c: number, d: number) => [number, number, number, number];
  readonly wasmembeddedpropagator_getState: (a: number) => [number, number, number];
  readonly wasmembeddedpropagator_getStateWithParams: (a: number) => [number, number, number];
  readonly wasmembeddedpropagator_getStep: (a: number) => bigint;
  readonly wasmembeddedpropagator_getTime: (a: number) => number;
  readonly wasmembeddedpropagator_getWidth: (a: number) => number;
  readonly wasmembeddedpropagator_isEmbedded: (a: number) => number;
  readonly wasmembeddedpropagator_new: (a: number, b: number, c: number, d: number) => [number, number, number];
  readonly wasmembeddedpropagator_newWithSpecies: (a: number, b: number, c: number, d: number, e: number, f: number) => [number, number, number];
  readonly wasmembeddedpropagator_reset: (a: number, b: number, c: number) => [number, number];
  readonly wasmembeddedpropagator_resetWithSpecies: (a: number, b: number, c: number, d: number, e: number) => [number, number];
  readonly wasmembeddedpropagator_run: (a: number, b: bigint) => void;
  readonly wasmembeddedpropagator_step: (a: number) => void;
  readonly wasmembeddedpropagator_totalMass: (a: number) => number;
