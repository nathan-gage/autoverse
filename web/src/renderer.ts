...
import type { SimulationState, SelectionRect, ViewerSettings, PresetRegion } from "./types";
...
export class Renderer {
  ...
  canvasToSim(canvasX: number, canvasY: number, simWidth: number, simHeight: number): { x: number; y: number } {
  ...
  private buildViridisMap(): Uint8ClampedArray {
    ...
    for (let i = 0; i < 256; i++) {
      ...
      map[i * 4 + 0] = Math.floor(viridisColors[low][0] + frac * (viridisColors[high][0] - viridisColors[low][0]));
      map[i * 4 + 1] = Math.floor(viridisColors[low][1] + frac * (viridisColors[high][1] - viridisColors[low][1]));
      map[i * 4 + 2] = Math.floor(viridisColors[low][2] + frac * (viridisColors[high][2] - viridisColors[low][2]));
      map[i * 4 + 3] = 255;
    }
    return map;
  }
